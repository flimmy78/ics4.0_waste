...
char              _MsgType[MSGLEN + 1];      /*  ÏûÏ¢ÀàÐÍ   */
...
int initChannel(int argc,char **argv)
{
    ...
    if (argc < 7)
    {
        usfErrlog("ÊäÈë²ÎÊý: <HostIp> <SendPort> <RecvPort> <LocalSendPort> <LocalRecvPort> <CrypSvrPort> [PreLen]");
        return (-1);
    }
...
/********************************************************************
  ...
  Description:    // ¿Í»§¶ËÒì²½³¤Á¬½ÓË«¹¤ÖÐ½ÓÊÕÊý¾ÝµÄÈë¿Úº¯Êý¡£Ö÷Òª¹¦ÄÜ£º
                  //      °ó¶¨±¾µØIPµØÖ·£¬½¨Á¢¼àÌý
                  //      ´´½¨½ÓÊÕÊý¾ÝµÄÏßË÷
  Calls:          // 
                  // recvThread
...
int clientLongAsync2In(int argc ,char ** argv)
{
    ...
    {
		usfErrlog("ÏûÏ¢ÀàÐÍ²ÎÊý³¬¹ý³¤¶È(%d)", MSGLEN);
		return (-1);
    }
...
/********************************************************************
  Function:       // recvThread
  Description:    // ¸Ãº¯ÊýÓÉclientLongAsync2Inº¯ÊýÆô¶¯µÄÏßË÷µ÷ÓÃ,
                  // Ö÷ÒªÍê³ÉÈçÏÂ¹¦ÄÜ£º
                  // Î¬»¤³¤Á¬½Ó
                  // ½ÓÊÕÏìÓ¦Êý¾Ý
  Calls:          // 
...
*/			
        ...
        */ 
        memset( sPartBuf, 0x0, MY_DATA_LEN+1);
        iPartLen = hsocketread( _RecvSockFd, sPartBuf,10);
        usfErrlog("Rcv_head[%d][%s]",iPartLen,sPartBuf);
        memset( sDestDat, 0x0, sizeof(sDestDat));       
        iDestLen=0;
        iBool=DoCypConvert(sDestDat,&iDestLen ,"01",sPartBuf,10 );
        if (iBool !=0)
        {
        ...
        memcpy(sRecvBuf,sDestDat,10);
        iRecvLen = 10;            
        memset(sMsgLen  ,0x0,sizeof(sMsgLen));
        memset(sMsgTm,0x0,sizeof(sMsgTm));        
        memcpy(sMsgLen,sDestDat,4);
        memcpy(sMsgTm,sDestDat+4,6);        
        iDatLen  = atoi(sMsgLen)-10;
        ...
        memcpy(sRecvBuf+10,sPartBuf,iDatLen);
        iRecvLen = 10+iDatLen;        
        memset(sDestDat,0x0,sizeof(sDestDat));
        iDestLen=0;
        ...
        usfErrlog("--Unc_Body[%d][%s]!", iDestLen,sDestDat);
		  ...
		  pMsg = (Message *)usfCreateMsg(_MsgType);
			if (pMsg == NULL)
			{
				usfErrlog("´´½¨ÏûÏ¢Ê§°Ü! IP=[%s] ÒÑ½ÓÊÕÊý¾Ý=[%d][%s]", _ServerIp, iDestLen, sDestDat);
				continue;
			}
		  usfTraceX("½ÓÊÕ±àºÅ=[%s] IP=[%s] Port=[%d]\n½ÓÊÕÊý¾Ý=[%d][%s]", pMsg->request_id, _ServerIp, _RecvPort, iDestLen, sDestDat);
...
/********************************************************************
  ...
  Description:    // Òì²½³¤Á¬½ÓË«¹¤ÖÐ·¢ËÍÇëÇóÊý¾ÝµÄÈë¿Úº¯Êý¡£Ö÷Òª¹¦ÄÜ£º
                  //      ½¨Á¢Óë·þÎñ¶ËµÄÁ¬½Ó
                  //      ·¢ËÍÊý¾Ý
...
int clientLongAsync2Out(Message * pMsg)
{
  ...
  /*usfErrlog("clientLongAsync2Out·¢ËÍÊý¾Ý:[%d][%s]",pMsg->msg_size, pMsg->msg_text); */

	/* ÐèÒªÅÐ¶ÏÊÇ·ñ±¨ÎÄ½âÎöÊ§°Ü, Èç¹ûÊÇ²»·¢ËÍ */
  usfErrlog("**********Enter_Send_Deal**********"); 
	...
	{
		/* Èç¹ûÊÇÇëÇó½»Ò×ÇÒÏûÏ¢Í·FLGÎª"1",ÐèÒª·µ»ØÍ¨Ñ¶½á¹û */
...
retry:
  ...
  usfErrlog("DoCypConvert_03_Rsp[%d][%s]",iBodyLen,sDestBody);
	...
	memcpy(sOrgHead,sDestBody,10);
	memset( sDestHead, 0x00, sizeof(sDestHead));
	iHeadLen=0;
  iBool=DoCypConvert(sDestHead,&iHeadLen ,"01",sOrgHead,10);		
	if (iBool !=0)
	{
  ...
  usfErrlog("DoCypConvert_ENCHEAD_RSP[%d][%s]",iHeadLen,sDestHead);	
	...
	memcpy(sOutMsg+2,sDestHead,10);
	memcpy(sOutMsg+12,sDestBody+10,iBodyLen-10);	
	iLen = hsocketwrite(_SendSockFd, sOutMsg, iOutLen);
  if (iLen != iOutLen)
  {
	  usfErrlog("ÇëÇó±àºÅ=[%s] ·¢ËÍÊý¾ÝÊ§°Ü! IP=[%s] Port=[%d]\nÊý¾Ý=[%d][%s]", pMsg->request_id, _ServerIp, _SendPort, iOutLen, sOutMsg);
	  usfErrlog("errno=[%d]",errno);
	  usfErrlog("socketwrite_len=[%d]",iLen);
	  hsocketclose(_SendSockFd);
	  _SendSockFd = -1;
	 ...
	 /* ÔÙ³¢ÊÔÁ¬½Ó1´Î */
	  if (iRetry == 0)
	  {
	    usfTraceX("Ô­ÓÐÁ¬½Ó·¢ËÍÊ§°Ü,³¢ÊÔ½¨Á¢ÐÂÁ¬½Ó!");
	    iRetry ++;
	    goto retry;
	  }
	  goto rp;
	...
	usfTraceX("·¢ËÍ±àºÅ=[%s] IP=[%s] Port=[%d]\n·¢ËÍÊý¾Ý=[%d][%s]", pMsg->request_id, _ServerIp, _SendPort, iOutLen, sOutMsg);
rp:
...
{	
  ...
  if (iBool ==0)
  {
	...
	usfErrlog("sDestBody=[%s]",sDestBody);
  memset( sOrgHead, 0x00, sizeof(sOrgHead));
  memcpy(sOrgHead,sDestBody,10);
  memset( sDestHead, 0x00, sizeof(sDestHead));
	usfErrlog("sOrgHead=[%s]",sOrgHead);
  iBool=rtss_header_code(sDestHead,&iHeadLen,sOrgHead,10,sMasterKey);
  if (iBool ==0)
  {
		usfErrlog("Err_DoSignOn_2[%s]",sOrgHead);
		return (0);
  }
	usfErrlog("sDestHead=[%s]",sDestHead);
  memset( sLeader, 0x00, sizeof(sLeader));
  sLeader[0]=0xef;
  sLeader[1]=0x14;
	memset( sOutMsg, 0x00, sizeof(sOutMsg));	
	...
	memcpy(sOutMsg,sLeader,2);
	memcpy(sOutMsg+2,sDestHead,10);
	memcpy(sOutMsg+12,sDestBody+10,iDestLen-10);	
	usfErrlog("sOutMsg=[%s]",sOutMsg);
...
int mytest()
{
    ...
    int  iDestLen=0;
    char sDestDat[MY_DATA_LEN + 1];    
    char sTestStr[MY_DATA_LEN + 1];

    char sMsgLen[5+1];
    char sMsgTm[6+1];

	  memset( sTestStr, 0x0, MY_DATA_LEN+1); 
	  memset( sPartBuf, 0x0, MY_DATA_LEN+1);
	  strcpy( sTestStr,"ï28?5<:928=SMT200?181  #) ( 20300;2747?919003908012584<0607908020300<016;;5:3  #) (   #8 :0093=096826;3(   #) (  239983163908000390800038200ì<¯");
		//strcpy( sTestStr,"ï28?5<?:4;2UE\230:709 ##,& (233563:777:?91033<608172=248637<608233564815;>091 ##,& ( ##=&280:3861>8155? ( ##,& ( 13<?0;153<608033<608033=488òâÈ-");
    memcpy( sPartBuf,sTestStr,1);
    iPartLen = 1;
    usfErrlog("Rcv_Leader1[%d][%x]!", iPartLen,sPartBuf[0]);
	  if ( (sPartBuf[0] !=0xef) && (sPartBuf[0] !=0x20) )
	  {
     ...
     memset( sPartBuf, 0x0, MY_DATA_LEN+1);
     memcpy( sPartBuf,sTestStr+1+1,10);
     iPartLen = 10;
     usfErrlog("Rcv_head[%d][%s]",iPartLen,sPartBuf);
     memset( sDestDat, 0x0, sizeof(sDestDat));       
     iDestLen=0;
     ...
     usfErrlog("iDatLen[%d]",iDatLen);        
     memset( sPartBuf, 0x0, MY_DATA_LEN+1);
     memcpy( sPartBuf, sTestStr+1+1+10, iDatLen);
     iPartLen = iDatLen;
...
int SignOk()
{
      ...
      int  iDestLen=0;
      char sDestDat[MY_DATA_LEN + 1];    
      char sMsgType[MSGLEN + 1];      /*  ÏûÏ¢ÀàÐÍ   */
      ...
      if (usfSendRequest(pMsg))
			{
				usfErrlog("SignOkÇëÇó±àºÅ=[%s] ·¢ËÍÇëÇóÏûÏ¢Ê§°Ü!", pMsg->request_id);
				usfDestroyMsg(pMsg);
				return(-1);				
			}
		  return(0);
