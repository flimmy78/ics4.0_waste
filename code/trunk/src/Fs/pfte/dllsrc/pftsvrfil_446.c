...
int clientSyncOut(Message * msg)
{
	...
	char		   		 aTmp[255+1];
	char					 aContFlag[2];
	char					 sFilNam[255+1],sFilSiz[16],sSekPnt[16], sSendHead[17+1];  /*¶ÁÈ¡µÄÎÄ¼þÃû,ÎÄ¼þ³¤¶È,Æ«ÒÆÎ»ÖÃ,ÐÅÏ¢Í·*/
...
retry:
	...
	if ( strncmp(msg->msg_text,"sfile",5) == 0 )  /*Èç¹û·¢ËÍÎÄ¼þÌåÐÅÏ¢Ôòµ¥¶À´¦ÀíÒª·¢ËÍµÄÐÅÏ¢*/
  {
	...
	if (send_buf != NULL)
	{
		...
		if( strncmp(msg->msg_text,"sfile",5) == 0 )  /*Èç¹û·¢ËÍÎÄ¼þÌåÐÅÏ¢Ôòµ¥¶À´¦ÀíÒª·¢ËÍµÄÐÅÏ¢*/
		{
			...
			if ( read_buf == NULL )
			{
				usfErrlog("ÇëÇó±àºÅ=[%s] ÉêÇëread_bufÄÚ´æ¿Õ¼äÊ§°Ü!", msg->request_id);
				hsocketclose(iSockFd);
				return(-1);
	  	}
	  	memset( read_buf, 0x0, sizeof( read_buf ));
			...
			{
				free(read_buf);
				usfErrlog("ÇëÇó±àºÅ=[%s] ¶ÁÈ¡ÎÄ¼þÊ§°Ü!", msg->request_id);
				return(-1);
			}
			memset( aTmp, 0x0, sizeof(aTmp));
			...
			usfErrlog("ÇëÇó±àºÅ=[%s] ·¢ËÍÊý¾Ý! IP=[%s] Port=[%d]\nÊý¾Ý³¤¶È=[%d][%s----%c%c%c%c%c%c----%c%c%c]", msg->request_id, aCommIp, iCommPort, iSendLen,send_buf,send_buf[19],send_buf[20],send_buf[21],send_buf[22],send_buf[23],send_buf[24],send_buf[iSendLen-3],send_buf[iSendLen-2],send_buf[iSendLen-1]);
	  }
	  else 
	  {
			sprintf(send_buf,"%c%c%s",0xef,0x14,msg->msg_text);
			usfErrlog("ÇëÇó±àºÅ=[%s] ·¢ËÍÊý¾Ý! IP=[%s] Port=[%d]\nÊý¾Ý=[%d][%s]", msg->request_id, aCommIp, iCommPort, iSendLen, send_buf);
	  }
	}
	else
  {
  ...
  if (iLen != iSendLen)
  {
  	usfErrlog("ÇëÇó±àºÅ=[%s] ·¢ËÍÊý¾ÝÊ§°Ü! IP=[%s] Port=[%d]\nÊý¾Ý=[%d][%s][%d]", msg->request_id, aCommIp, iCommPort, iSendLen, send_buf,iLen);
		hsocketclose(iSockFd);
		if (send_buf != NULL)
		{
			free(send_buf);
