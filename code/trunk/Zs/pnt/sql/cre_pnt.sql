...
--状态( U-初始 B-记帐成功(确认) S-转发成功 X-系统错误(未发送) F-发送失败 T-发送超时 R-抹账 C-冲正未确认 D-已冲正 A-柜面记帐成功)
